...
//// import
// com.ppdai.data.goldeye.dashboard.domain.dataaccess.strategymanage.AlarmDingDingDataAccess;
//
...
// // 用户权限是否已存在
// private static String selectUserAuthByCodeSql = "SELECT COUNT(*) FROM TB_USER_AUTH WHERE
// AUTH_CODE = ? AND USER_CODE = ? AND ISACTIVE = 1";
//
// // 新增用户权限
// private static String createUserAuthSql = "INSERT INTO
// TB_USER_AUTH(USER_CODE,USER_NAME,AUTH_CODE,AUTH_NAME) VALUES (?,?,?,?)";
//
// // 删除用户权限
// private static String deleteUserAuthSql = "UPDATE TB_USER_AUTH SET ISACTIVE = 0 WHERE
// PK_USER_AUTH_CODE = ?";
//
// // 修改用户权限
// private static String updateUserAuthSql = "UPDATE TB_USER_AUTH SET ISACTIVE = ? WHERE
// PK_USER_AUTH_CODE = ?";
//
...
//
// return i == 1 ? new ResultResponse(true, UserReponseEnum.SUCCEED.getResCode(),
// UserReponseEnum.SUCCEED.getResMsg())
...
// int i = mysqlJdbcTemplate.update(updateUserAuthSql, userAuthDto.getIsActive(),
// userAuthDto.getPkUserAuthCode());
//
// return i == 1 ? new ResultResponse(true, UserReponseEnum.SUCCEED.getResCode(),
// UserReponseEnum.SUCCEED.getResMsg())
// : new ResultResponse(false, UserReponseEnum.FAIL.getResCode(), UserReponseEnum.FAIL.getResMsg());
...
// StringBuffer selectUserAuthSql = new StringBuffer("SELECT PK_USER_AUTH_CODE, USER_CODE,
// USER_NAME, AUTH_CODE, AUTH_NAME, CREATE_USER, UPDATE_USER, INSERTTIME, UPDATETIME, ISACTIVE FROM
// TB_USER_AUTH WHERE 1 = 1 ");
...
// UserAuthDto userAuthDto = new UserAuthDto(rs.getInt(1), rs.getString(2), rs.getString(3),
// rs.getString(4), rs.getString(5),
// rs.getString(6), rs.getString(7),
// DateUtil.formatDate(rs.getString(8)),DateUtil.formatDate(rs.getString(9)), rs.getByte(10));
...
// if (count != 0) {
// return new ResultResponse(false, UserReponseEnum.USER_AUTH_EXIST.getResCode(),
// UserReponseEnum.USER_AUTH_EXIST.getResMsg());
// }
...
//// List<AlarmDingDingDto>
// alarmDingDingDtoList=alarmDingDingDataAccess.selectDingUserByParam(alarmDingDingDto);
...
//// }else {
//// if ((userDtoList == null || userDtoList.size() == 0) && (alarmDingDingDtoList == null ||
// alarmDingDingDtoList.size() == 0)) {
//// return new ResultResponse(false, UserReponseEnum.FAIL.getResCode(), "此用户与钉钉都不存在");
//// } else if (userDtoList != null && userDtoList.size() != 0) {
//// i = mysqlJdbcTemplate.update(createUserAuthSql, new Object[]{userAuthDto.getUserCode(),
// userDtoList.get(0).getUserName(), userAuthDto.getAuthCode(), authDtoList.get(0).getAuthName()});
//// } else if (alarmDingDingDtoList != null && alarmDingDingDtoList.size() != 0) {
//// i = mysqlJdbcTemplate.update(createUserAuthSql, new Object[]{userAuthDto.getUserCode(),
// alarmDingDingDtoList.get(0).getDingName(), userAuthDto.getAuthCode(),
// authDtoList.get(0).getAuthName()});
//// } else {
//// }
//// }
// return i == 1 ? new ResultResponse(true, UserReponseEnum.SUCCEED.getResCode(),
