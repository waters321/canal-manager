...
public class ProcessMonitorService {
  ...
  public List<DestinationsConfig> selectDestinationConfigByArgs(Map<String, String> argsMap) {
    ...
    // 拼接sql
    String temp =
        "select id,canal_id,destination_name,description,destination_configuration,standby_configuration,"
  ...
  public Map<String,Object> listCanalServerConfig(Map<String, String> argsMap) {
    ...
    // 拼接sql
    String temp =
        "select id,canal_server_type,canal_home,canal_server_name,canal_server_host,canal_server_port,canal_server_configuration,"
    ...
    if (StringUtils.isNotBlank(canalServerName)) {
      selectByParamSql.append(" and canal_server_name like '%").append(canalServerName)
          .append("%' ");
    }
    if (StringUtils.isNotBlank(canalServerHost)) {
      selectByParamSql.append(" and canal_server_host like '%").append(canalServerHost)
          .append("%' ");
    }
  ...
  public DestinationsConfig selectDestinationsConfigByArgs(Map<String, String> argsMap) {
    ...
    try {
      destinationConfig =
          (DestinationsConfig) mysqlJdbcTemplate.queryForObject(selectByParamSql.toString(),
              new BeanPropertyRowMapper<DestinationsConfig>(DestinationsConfig.class));
    } catch (Exception e) {
   ...
   */
  public List<CanalClientStatus> getCanalClientStatusBeanList(String canalId,
    ...
    /*
     * 判断canal client的状态，分2步 第一步：先循环zk取的数据 把从zk取的数据做比较基准，数据都是zk存在的。 循环每个destination.
    ...
    for (ZKDestinationBean zKDestinationBean : zKDestinationBeanList) {
      ...
      if (zKDestinationBean.getzKDestinationRunningNode() == null) {
        logger.debug("destinationName：" + destinationName
            + "没有server运行它,有可能该destination已经摘除，zk信息过时，不计算，直接跳过");
        continue;
      }
      ...
      ZKDestinationClientRunningNode clientRunningNode =
          zKDestinationBean.getzKDestinationClientRunningNode();
      if (clientRunningNode != null) {// 说明该destination有客户端消费
        ...
        if (destinationsConfig != null) {// 若数据库中有该destination的信息
          ...
          String mysqlAddress = CanalPropertyUtils.getPropertyValueByKey(destinationConfiguration,
          ...
          StringBuilder comment = new StringBuilder();
          comment.append("destination:" + destinationName
              + "有client消费,但db中没有该destination信息，说明zk与数据库信息不一致，可能数据库没配置该destination");
          String clientAddressPort = clientRunningNode.getAddress();
          clientStatus.setClientAddressPort(clientAddressPort);
        ...
        if (destinationsConfig != null) {// 若数据库中有该destination的信息
          ...
          if (clientConfig != null) {
            ...
            comment
                .append("destination:" + destinationName + "没有client消费,数据库没有配置client或配置了多个client");
          }
          // 取配置文件，从配置文件取对应的mysql的地址
          String destinationConfiguration = destinationsConfig.getDestinationConfiguration();
          String mysqlAddress = CanalPropertyUtils.getPropertyValueByKey(destinationConfiguration,
          ...
          StringBuilder comment = new StringBuilder();
          comment.append("destination:" + destinationName
              + "没有client消费,且db中没有该destination信息，可能数据库配置该destination有误");
          clientStatus.setColor(CanalConstants.YELLOW);
   ...
   */
  public List<CanalServerConfigShow> getCanalServerStatusBeanList(
      List<CanalServerConfigShow> canalServerConfigShowList) {
    ...
    for (CanalServerConfigShow canalServerConfig : canalServerConfigShowList) {
      ...
      String masterCanalIp = CanalPropertyUtils
          .getPropertyValueByKey(canalServerConfig.getCanalServerConfiguration(), "canal.ip");
      String standbyCanalIp = CanalPropertyUtils
          .getPropertyValueByKey(canalServerConfig.getStandbyServerConfiguration(), "canal.ip");
