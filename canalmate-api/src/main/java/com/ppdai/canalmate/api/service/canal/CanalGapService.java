...
public class CanalGapService {
  ...
  public List<CanalGapDto> listInstanceStatus(String canalId) {
    ...
    CanalServerConfig canalServerConfig =
        processMonitorService.selectCanalServerConfigByPrimaryKey(Long.valueOf(canalId));
    ...
    try {
      ...
      String destinationName, masterMysqlSlaveId, standbyMysqlSlaveId, masterAddress, dbUsername,
          dbPassword, masterJournalName, masterPosition;
      ...
      for (DestinationsConfig destinationsConfig : destinationsConfigList) {
        String positionGap = null, instanceJournalName = null, instancePosition = null,
        ...
        masterMysqlSlaveId = CanalPropertyUtils.getPropertyValueByKey(destinationConfiguration,
            "canal.instance.mysql.slaveId");
        standbyMysqlSlaveId = CanalPropertyUtils.getPropertyValueByKey(standbyConfiguration,
            "canal.instance.mysql.slaveId");
        ...
        masterAddress = CanalPropertyUtils.getPropertyValueByKey(destinationConfiguration,
            "canal.instance.master.address");
        dbUsername = CanalPropertyUtils.getPropertyValueByKey(destinationConfiguration,
            "canal.instance.dbUsername");
        dbPassword = CanalPropertyUtils.getPropertyValueByKey(destinationConfiguration,
            "canal.instance.dbPassword");

        // 登录mysql查询mysql位点
        Map<String, String> result =
            DBUtils.getDBMasterStatus(masterAddress, dbUsername, dbPassword);
        ...
        ZKDestinationBean zKDestinationBean = CanalZKUtils
        ...
        if (zKDestinationBean != null) {// 若该dest能从zk正确获取
          ...
          if (logPosition != null) {
            ...
            commentSb.append("查询destination为" + destinationName + "在zk的位点信息失败,"
                + zKDestinationBean.getStatusComment());
            colour = CanalConstants.RED;
          }
        ...
        if (!StringUtils.isBlank(colour)) {// 若color不为空，说明前面赋值为红，说明已经有问题，不用判断
          ...
          // 计算gap
          positionGap =
              String.valueOf(Long.valueOf(masterPosition) - Long.valueOf(instancePosition));// 计算gap
          ...
          logger.debug(
              "gap_second:" + (currentTimestamp - destinationTimestamp) + ",current_timestamp:"
                  + currentTimestamp + ",destination_timestamp:" + destinationTimestamp);
          logger.debug("gap_hour:" + gapHour);
          // 先判断是否超过了小时级别告警，若超过小时级别，则直接告警，后面分钟级别不判断。若没超过小时级别，再判断是否超过分钟级别。
          if (gapHour > GAP_HOUR_threshold) {
            commentSb
                .append("client位点时间比当前时间已延迟" + gapHour + "小时,超过" + GAP_HOUR_threshold + "小时阈值,");
