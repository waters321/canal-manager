...
public class UserRoleDataAccess {
  ...
  // 角色是否已存在
  private static String selecUserRoleByCodeSql =
      "select count(*) from  tb_user_role where user_code = ? and role_code = ?  and isactive = 1";

  // 新增角色
  private static String createUserRoleSql =
      "insert into tb_user_role(user_code,user_name,role_code,role_name,create_user) values (?,?,?,?,?)";

  // 通过用户code删除角色
  private static String deleteUserRoleSqlByUserCode =
      "delete from tb_user_role where user_code = ?";

  // 删除角色
  private static String deleteUserRoleSql =
      "update tb_user_role set update_user = ?, isactive = 0  where pk_user_role_code = ?";

  // 修改
  private static String updateUserRoleSql =
      "update tb_user_role set update_user = ?, isactive = ?  where pk_user_role_code = ?";

  // 物理删除用户的角色
  public ResultResponse deleteUserRoleByUserCode(UserRoleDto userRoleDto) {
    int i = mysqlJdbcTemplate.update(deleteUserRoleSqlByUserCode,
        new Object[] {userRoleDto.getUserCode()});

    return i == 1
        ? new ResultResponse(true, RoleReponseEnum.SUCCEED.getResCode(),
            RoleReponseEnum.SUCCEED.getResMsg())
        : new ResultResponse(false, RoleReponseEnum.FAIL.getResCode(),
            RoleReponseEnum.FAIL.getResMsg());
  }

  public ResultResponse deleteUserRole(UserRoleDto userRoleDto) {
    int i = mysqlJdbcTemplate.update(deleteUserRoleSql,
        new Object[] {userRoleDto.getUpdateUser(), userRoleDto.getPkUserRoleId()});
  ...
  public ResultResponse updateUserRole(UserRoleDto userRoleDto) {
    int i = mysqlJdbcTemplate.update(updateUserRoleSql, new Object[] {userRoleDto.getUpdateUser(),
        userRoleDto.getIsActive(), userRoleDto.getPkUserRoleId()});
  ...
  public List<UserRoleDto> selectRole(UserRoleDto userRoleDto) {
    ...
    StringBuffer selectRoleSql = new StringBuffer(
        "select pk_user_role_code, user_code, user_name, role_code, role_name, create_user, update_user, inserttime, updatetime, isactive from tb_user_role where 1 = 1 ");
    ...
    if (StringUtils.isNotBlank(userRoleDto.getUserCode())) {
      selectRoleSql.append(" and user_code like '%").append(userRoleDto.getUserCode())
          .append("%' ");
    }
    if (StringUtils.isNotBlank(userRoleDto.getUserName())) {
      selectRoleSql.append(" and user_name like '%").append(userRoleDto.getUserName())
          .append("%' ");
    }
    ...
    if (StringUtils.isNotBlank(userRoleDto.getRoleName())) {
      selectRoleSql.append(" and role_name like '%").append(userRoleDto.getRoleName())
          .append("%' ");
    }
    ...
    mysqlJdbcTemplate.query(selectRoleSql.toString(), new RowCallbackHandler() {
      ...
      public void processRow(ResultSet rs) throws SQLException {
        UserRoleDto userRoleDto1 = new UserRoleDto(rs.getInt(1), rs.getString(2), rs.getString(3),
            rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7),
            DateUtil.formatDate(rs.getString(8)), DateUtil.formatDate(rs.getString(9)),
            rs.getByte(10));
  ...
  public ResultResponse insertUserRole(UserRoleDto userRoleDto) {
    ...
    if (count != 0) {
      return new ResultResponse(false, RoleReponseEnum.EXIST.getResCode(),
          RoleReponseEnum.EXIST.getResMsg());
    }
    ...
    if (userDtoList == null || userDtoList.size() == 0) {
      return new ResultResponse(false, RoleReponseEnum.USER_NOT_EXIST.getResCode(),
          RoleReponseEnum.USER_NOT_EXIST.getResMsg());
    }

    if (roleDtoList == null || roleDtoList.size() == 0) {
      return new ResultResponse(false, RoleReponseEnum.ROLE_NOT_EXIST.getResCode(),
          RoleReponseEnum.ROLE_NOT_EXIST.getResMsg());
    }

    int i = mysqlJdbcTemplate.update(createUserRoleSql,
        new Object[] {userRoleDto.getUserCode(), userDtoList.get(0).getUserName(),
            userRoleDto.getRoleCode(), roleDtoList.get(0).getRoleName(),
            userRoleDto.getCreateUser()});
  ...
  public ResultResponse deleteByIds(String[] ids, String userCode) {
    ...
    String deleteUserRoleByIdsSql =
        "update tb_user_role set isactive = 0,update_user = ? where pk_user_role_code in (";
  ...
  public ResultResponse openByIds(String[] ids, String userCode) {
    ...
    String openUserRoleByIdsSql =
        "update tb_user_role set isactive = 1,update_user=? where pk_user_role_code in (";
    for (int i = 0; i < ids.length - 1; i++) {
