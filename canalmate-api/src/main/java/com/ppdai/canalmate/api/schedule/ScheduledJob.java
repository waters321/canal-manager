...
public class ScheduledJob {
  ...
  private static final SimpleDateFormat DECIMAL_FORMAT =
      new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

  private Integer count0 = 0;
  public final static long ONE_MINUTE = 60 * 1000;
  public final static long CHECK_INTERVAL_MINUTE = 1 * ONE_MINUTE; // 每10分钟调度一次检查进程

  /*
   * 间隔：每隔 fixedRate 时间 ，该程序就出发一次。 作用：监控canal server 和canal client,client
   * gap的进程状态，并把信息封装成canalWarn，发送到管理员角色的用户的邮箱中。
   */
  @SuppressWarnings("unchecked")
  @Scheduled(fixedRate = CHECK_INTERVAL_MINUTE)
  public void checkServerStatus() throws InterruptedException {
    logger
        .info(String.format("---第%s次调度执行开始，当前时间为：%s", count0++, DECIMAL_FORMAT.format(new Date())));
    ...
    /*
     * 循环所有的Server状态。针对每一个server 1：先检查server的状态，并封装信息为报警信息 2：检查该server下的所有client信息，并封装为报警信息
    ...
    for (CanalServerConfigShow canalServerConfigShow : canalServerConfigShowList) {
      ...
      CanalServerConfig canalServerConfig =
          processMonitorService.selectCanalServerConfigByPrimaryKey(canalId);
      ...
      List<CanalClientStatus> canalClientStatusList = processMonitorService
          .getCanalClientStatusBeanList(String.valueOf(canalId), zKDestinationBeanList);
      ...
      List<CanalGapDto> instanceGapList =
          canalGapService.listInstanceStatus(String.valueOf(canalId));
    ...
    logger.info(String.format("---第%s次执行调度结束，当前时间为：%s，调度耗时:%s s", count0++,
        DECIMAL_FORMAT.format(new Date()), (t2 - t1) / 1000));
  }
  ...
  private void sendMail(List<CanalWarn> canalWarnList) {
    ...
    if (StringUtils.isNotBlank(isSendEmail) && isSendEmail.equals("1")
        && StringUtils.isNotBlank(contentSb.toString())) {
