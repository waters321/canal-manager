...
public class CanalInstanceService {
  ...
  // @Autowired
  // private TbCanalinstanceConfigMapper tbCanalinstanceConfigMapper;//
  // 通过myBatis的方式操作数据库,可以和上面的jdbcTemplate的方式共存


  public List<CanalInstanceConfigDto> selectCanalInstanceConfig(Map<String, String> argsMap) {
    String temp =
        "select instance_name, mysql_slaveId, master_address, master_journal_name, master_position, "
  ...
  // public List<TbCanalinstanceConfig> selectCanalInstanceConfigByMybatis(TbCanalinstanceConfig
  // example) {
  ...
  public ResultResponse insertCanalInstanceConfig(CanalInstanceConfigDto canalInstanceConfig) {
    ...
    //
    String insertsql =
        "insert into tb_canalinstance_config(instance_name, mysql_slaveId, master_address, dbUsername, dbPassword, filter_regex) values (?,?,?,?,?,?)";
    int i = mysqlJdbcTemplate.update(insertsql,
        new Object[] {canalInstanceConfig.getInstanceName(), canalInstanceConfig.getMysqlSlaveId(),
            canalInstanceConfig.getMasterAddress(), canalInstanceConfig.getDbUsername(),
            canalInstanceConfig.getDbPassword(), canalInstanceConfig.getFilterRegex()});
