...
public class MenuDataAccess {
  ...
  // 菜单是否已存在
  private static String selectMenuByCodeSql =
      "SELECT COUNT(*) FROM  TB_MENU WHERE MENU_CODE = ?  AND ISACTIVE = 1";

  // 新增菜单
  private static String createMenuSql =
      "INSERT INTO TB_MENU(MENU_CODE,MENU_NAME,SUPERIOR_MENU,MENU_URL,MENU_SERIAL) VALUES (?,?,?,?,?)";

  // 删除菜单
  private static String deleteMenuSql = "UPDATE TB_MENU  SET ISACTIVE = 0 WHERE PK_MENU_ID = ?";

  private static String updateMenuSql =
      "UPDATE TB_MENU  SET SUPERIOR_MENU = ?, MENU_URL = ?, MENU_SERIAL = ?, ISACTIVE = ? WHERE PK_MENU_ID = ?";
  ...
  public ResultResponse insertMenu(MenuDto menuDto) {
    // 判断是否已存在
    int count =
        mysqlJdbcTemplate.queryForObject(selectMenuByCodeSql, Integer.class, menuDto.getMenuCode());
    if (count != 0) {
      return new ResultResponse(false, ReponseEnum.MENU_EXIST.getResCode(),
          ReponseEnum.MENU_EXIST.getResMsg());
    }
    // 新增
    int i = mysqlJdbcTemplate.update(createMenuSql,
        new Object[] {menuDto.getMenuCode(), menuDto.getMenuName(), menuDto.getSuperiorMenu(),
            menuDto.getMenuUrl(), menuDto.getMenuSerial()});
    return i == 1
  ...
  public ResultResponse updateMenu(MenuDto menuDto) {
    int i = mysqlJdbcTemplate.update(updateMenuSql,
        new Object[] {menuDto.getSuperiorMenu(), menuDto.getMenuUrl(), menuDto.getMenuSerial(),
            menuDto.getIsActive(), menuDto.getPkMenuId()});
    return i == 0
        ? new ResultResponse(false, ReponseEnum.FAIL.getResCode(), ReponseEnum.FAIL.getResMsg())
        : new ResultResponse(true, ReponseEnum.SUCCEED.getResCode(),
            ReponseEnum.SUCCEED.getResMsg());
  ...
  public List<MenuDto> selectMenu(MenuDto menuDto) {
    ...
    StringBuffer selectMenuSql = new StringBuffer(
        "SELECT PK_MENU_ID, MENU_CODE, MENU_NAME, SUPERIOR_MENU, MENU_URL, MENU_SERIAL, CREATE_USER, UPDATE_USER, INSERTTIME, UPDATETIME, ISACTIVE FROM TB_MENU WHERE 1 = 1 ");
    ...
    if (StringUtils.isNotBlank(menuDto.getSuperiorMenu())) {
      selectMenuSql.append(" AND SUPERIOR_MENU LIKE '%").append(menuDto.getSuperiorMenu())
          .append("%' ");
    }
    ...
    mysqlJdbcTemplate.query(selectMenuSql.toString(), new RowCallbackHandler() {
      ...
      public void processRow(ResultSet rs) throws SQLException {
        MenuDto menuDto = new MenuDto(rs.getInt(1), rs.getString(2), rs.getString(3),
