...
public class UserDataAccess {
  ...
  private static String selectUserMenuAuthSql =
      "select t.menu_code,t.menu_name, t.menu_url, t.menu_icon, t.superior_menu, t.menu_serial \r\n"
          + "from tb_menu t\r\n" + "where menu_code in(\r\n" + "select menu_code\r\n"
  ...
  // 用户是否已存在
  private static String selectUserByCodeSql =
      "select count(*) from  tb_user where user_code = ? and isactive = 1";
  private static String selectUserIgnoreActiveByCodeSql =
      "select count(*) from  tb_user where user_code = ? ";

  // 用户是否已存在(登录)
  private static String selectUserByCodePasswordSql =
      "select count(*) from  tb_user where user_code = ? and user_password=?";

  // 用户是否是管理员,管理员的role_code是从数据库查的，目前写死
  private static String selectUserIsAdmin =
      "select count(*) from tb_user_role where user_code=? and role_code=1";

  // 新增用户
  private static String createUserSql =
      "insert into tb_user(user_code, user_name, user_password, user_mobile, user_email, superior) values (?,?,?,?,?,?)";

  private static String deleteUserSql = "update tb_user set isactive = 0 where pk_user_id = ?";

  /**
   * 根据角色编号查菜单权限 role_code=1:管理员，role_code=2:普通用户
   * 
   * @param userCode
   * @return
   */
  public List<MenuDto> selectUserMenuAuth(String role_code) {
    ...
    mysqlJdbcTemplate.query(selectUserMenuAuthSql, new Object[] {role_code},
        new RowCallbackHandler() {
          ...
          public void processRow(ResultSet rs) throws SQLException {
            ...
            MenuDto menu =
                new MenuDto(menuCode, menuName, superiorMenu, menuUrl, menuIcon, menuSerial);
  ...
  public UserDto selectUserById(Integer userId) {
    StringBuffer selectUserByParamSql = new StringBuffer(
        "select pk_user_id, user_code,user_name,user_mobile,user_email,superior,create_user,update_user,inserttime,updatetime ,isactive from  tb_user where 1 = 1");
    ...
    mysqlJdbcTemplate.query(selectUserByParamSql.toString(), new RowCallbackHandler() {
      ...
      public void processRow(ResultSet rs) throws SQLException {
        UserDto user = new UserDto(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
            ...
            DateUtil.formatDate(rs.getString(9)), DateUtil.formatDate(rs.getString(10)),
            rs.getByte(11));
        userList.add(user);
      }
    });
  ...
  public List<UserDto> selectUserByParam(UserDto userDto) {
    ...
    if (StringUtils.isNotBlank(userDto.getUserName())) {
      selectUserByParamSql.append(" and user_name like '%").append(userDto.getUserName())
          .append("%' ");
    }
    if (StringUtils.isNotBlank(userDto.getSuperior())) {
      selectUserByParamSql.append(" and superior like '%").append(userDto.getSuperior())
          .append("%' ");
    }
  ...
  public ResultResponse insertUser(UserDto user) {
    // 判断是否已注册
    int count = mysqlJdbcTemplate.queryForObject(selectUserIgnoreActiveByCodeSql, Integer.class,
        user.getUserCode());
    if (count != 0) {
    ...
    // 注册
    int i = mysqlJdbcTemplate.update(createUserSql,
        new Object[] {user.getUserCode(), user.getUserName(), user.getUserPassword(),
            user.getUserMobile(), user.getUserEmail(), user.getSuperior()});


    return i == 1
        ? new ResultResponse(true, ReponseEnum.SUCCEED.getResCode(),
            ReponseEnum.SUCCEED.getResMsg())
  ...
  public ResultResponse updateUser(UserDto user) {
    ...
    // USER_PASSWORD = ?,USER_MOBILE = ?,USER_EMAIL = ?,SUPERIOR = ? WHERE USER_CODE = ? AND
    // ISACTIVE = 1
    ...
    updateUserSql.deleteCharAt(updateUserSql.length() - 1).append(" where user_code = '")
        .append(user.getUserCode()).append("'");
  ...
  public LoginResponse login(String userCode, String password) {
    ...
    if (count == 0) {
      return new LoginResponse(false, ReponseEnum.USER_NOT_EXIST.getResCode(),
          ReponseEnum.USER_NOT_EXIST.getResMsg());
    }
    // 检查用户密码是否正确
    Integer i = mysqlJdbcTemplate.queryForObject(selectUserByCodePasswordSql, Integer.class,
    ...
    // 到这里，用户已经登录成功，检查用户角色
    Integer j =
        mysqlJdbcTemplate.queryForObject(selectUserIsAdmin, Integer.class, new Object[] {userCode});
    String isAdmin = "0";// 默认不是admin角色
    if (j > 0) {
      isAdmin = "1";
    }
    LoginResponse loginResponse =
        new LoginResponse(true, ReponseEnum.SUCCEED.getResCode(), ReponseEnum.SUCCEED.getResMsg());
