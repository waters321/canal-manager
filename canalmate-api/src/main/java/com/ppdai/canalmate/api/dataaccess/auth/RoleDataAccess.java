...
public class RoleDataAccess {
  ...
  // 角色是否已存在
//  private static String selecRoleByCodeSql =
//      "SELECT COUNT(*) FROM  TB_GOLDEYE_ROLE WHERE ROLE_CODE = ? AND ALARM_CODE = ?  AND ISACTIVE = 1";

  // 新增角色
//  private static String createRoleSql =
//      "INSERT INTO TB_GOLDEYE_ROLE(ROLE_CODE,ROLE_NAME,ALARM_CODE,ALARM_NAME,CREATE_USER) VALUES (?,?,?,?,?)";

  // 删除角色
  private static String deleteRoleSql =
      "UPDATE TB_GOLDEYE_ROLE SET UPDATE_USER = ? ,ISACTIVE = 0 WHERE PK_ROLE_CODE = ?";

  // 修改用户权限
  private static String updateRoleSql =
      "UPDATE TB_GOLDEYE_ROLE SET UPDATE_USER = ? , ISACTIVE = ? WHERE PK_ROLE_CODE = ?";


  public ResultResponse deleteRole(RoleDto roleDto) {
    int i = mysqlJdbcTemplate.update(deleteRoleSql,
        new Object[] {roleDto.getUpdateUser(), roleDto.getPkRoleId()});
  ...
  public ResultResponse updateRole(RoleDto roleDto) {
    int i = mysqlJdbcTemplate.update(updateRoleSql,
        new Object[] {roleDto.getUpdateUser(), roleDto.getIsActive(), roleDto.getPkRoleId()});
  ...
  public List<RoleDto> selectRole(RoleDto roleDto) {
    ...
    StringBuffer selectRoleSql = new StringBuffer(
        "select pk_role_code, role_code, role_name, create_user, update_user, inserttime, updatetime, isactive from tb_role where 1 = 1 ");
  ...
  // e.printStackTrace();
  // return new ResultResponse(false, RoleReponseEnum.ALARMCODE_NOT_NUMBER.getResCode(),
  // RoleReponseEnum.ALARMCODE_NOT_NUMBER.getResMsg());
  // }
  // alarmScriptDto.setIsActive(Byte.valueOf("1"));
  // List<AlarmScriptDto> alarmScriptDtoList =
  // alarmScriptDataAccess.selectAlarmScript(alarmScriptDto);
  //
  ...
  // List<AlarmScriptDictDto> alarmScriptDictDtoList =
  // alarmScriptTypedictDataAccess.selectAlarmScriptTypedict(alarmScriptDictDto);
  //
  // if (alarmScriptDtoList == null || alarmScriptDtoList.size() == 0) {
  // return new ResultResponse(false, RoleReponseEnum.ALARM_NOT_EXIST.getResCode(),
  // RoleReponseEnum.ALARM_NOT_EXIST.getResMsg());
  // }
  //
  // if (alarmScriptDictDtoList == null || alarmScriptDictDtoList.size() == 0) {
  // return new ResultResponse(false, RoleReponseEnum.ALARBUSINESS_NOT_EXIST.getResCode(),
  // RoleReponseEnum.ALARBUSINESS_NOT_EXIST.getResMsg());
  // }
  //
  // int i = mysqlJdbcTemplate.update(createRoleSql, new Object[]{roleDto.getRoleCode(),
  // alarmScriptDictDtoList.get(0).getValue(), roleDto.getAlarmCode(),
  // alarmScriptDtoList.get(0).getDescri(), roleDto.getCreateUser()});
  //
  // return i == 1 ? new ResultResponse(true, RoleReponseEnum.SUCCEED.getResCode(),
  // RoleReponseEnum.SUCCEED.getResMsg())
  // : new ResultResponse(false, RoleReponseEnum.FAIL.getResCode(),
  // RoleReponseEnum.FAIL.getResMsg());
  //
  ...
  public ResultResponse deleteByIds(String[] ids, String userCode) {
    ...
    String deleteRoleByIdsSql =
        "UPDATE TB_GOLDEYE_ROLE SET ISACTIVE = 0,UPDATE_USER = ? WHERE PK_ROLE_CODE IN (";
  ...
  public ResultResponse openByIds(String[] ids, String userCode) {
    ...
    String openRoleByIdsSql =
        "UPDATE TB_GOLDEYE_ROLE SET ISACTIVE = 1,UPDATE_USER = ? WHERE PK_ROLE_CODE IN (";
    for (int i = 0; i < ids.length - 1; i++) {
