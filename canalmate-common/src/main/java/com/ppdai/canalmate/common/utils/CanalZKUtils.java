...
public class CanalZKUtils {
  ...
  /*
   * 根据传入的zk地址，查询该zk对应的整个canal server的集群列表。 [zk: localhost:2181(CONNECTED) 2] ls
   * /otter/canal/cluster [IP:11111]
   */
  public static List<String> getZKCanalServerListFromZKAddress(String zkAddress) {
    ...
    try {
      ...
      List<String> canalServerList =
          test.getChildren(ZookeeperPathUtils.CANAL_CLUSTER_ROOT_NODE, false);
  ...
  /*
   * 一组canal server有2个canal server，一个active,一个standby，对应一个zk上的地址。 该方法传入一个zk地址，得到该地址下该一组canal
   * server(一个active,一个standby)对应的一个ZKDestinationBean的列表 列表的每个ZKDestinationBean，对应一个该canal server
   * 对外服务的 destination,和相关的canal server ，和相关的client node信息 这个方法重要。
   */
  public static List<ZKDestinationBean> getZKDestinationBeanListFromZKAddress(String zkAddress) {
    ...
    try {
      ...
      List<String> destinationList =
          test.getChildren(ZookeeperPathUtils.DESTINATION_ROOT_NODE, false);
      for (String destinationName : destinationList) {
        ZKDestinationBean bean =
            getZKDestinationBeanFromZKAddressByDestinationName(zkAddress, destinationName);
  ...
  /*
   * 根据传入的zk地址和一个destinameName。 返回： 该destination对应的 ZKDestinationBean 对象 该 ZKDestinationBean 包括：
   ...
   */
  public static ZKDestinationBean getZKDestinationBeanFromZKAddressByDestinationName(
      String zkAddress, String destinationName) {
    ...
    try {
      ...
      String DESTINATION_RUNNING_NODE_PATH =
          ZookeeperPathUtils.getDestinationServerRunning(destinationName);
      ...
      if (result == null) {
        // P.p("=============没有server运行该dest,DESTINATION_RUNNING_NODE
        // data:"+DESTINATION_RUNNING_NODE_PATH);
      ...
      List<ZKDestinationClusterNode> zKDestinationClusterNodeList =
          new ArrayList<ZKDestinationClusterNode>();
      String DESTINATION_CLUSTER_ROOT_PATH =
          ZookeeperPathUtils.getDestinationClusterRoot(destinationName);
      // P.p("=============DESTINATION_CLUSTER_ROOT_PATH:"+DESTINATION_CLUSTER_ROOT_PATH);
      List<String> destinationCanalServerList =
          test.getChildren(DESTINATION_CLUSTER_ROOT_PATH, false);
      ...
      ZKDestinationClientRunningNode zKDestinationClientRunningNode =
          new ZKDestinationClientRunningNode();
      String clientRunningNodePath =
          ZookeeperPathUtils.getDestinationClientRunning(destinationName, CanalConstants.clientId);
      ...
      if (result == null) {
        ...
        zKDestinationClientRunningNode =
            JSON.parseObject(result, ZKDestinationClientRunningNode.class);
      }
      ...
      String clientCursorPath =
          ZookeeperPathUtils.getCursorPath(destinationName, CanalConstants.clientId);
      result = test.getNodeData(clientCursorPath, false);
      // P.p("====clientCursorPath:"+clientCursorPath+",result:"+result);
      if (result == null) {
        statusCommentSb.append(
            ",获取dest:" + destinationName + "在zk的消费位点失败,clientCursorPath:" + clientCursorPath);
        position = null;
      } else {
