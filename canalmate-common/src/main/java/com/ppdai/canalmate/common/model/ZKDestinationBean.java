...
/*
 * 描述一个destination在zk中的状态信息 [zk: localhost:2181(CONNECTED) 139] ls
 * /otter/canal/destinations/ppdai_user [running, cluster, 1001] 基本结构： ZKDestinationBean
 * destinationName destination名称，对应canal/conf下面的目录 destinationPath 该destination在zk中的path
 * zKDestinationClusterNodePath 该destination对应的canal server 集群，对应zk的路径 ： ls
 * /otter/canal/destinations/ppdai_user/cluster ，输出：[IP:11111, IP:11111] ZKDestinationRunningNode
 * zk命令：get /otter/canal/destinations/ppdai_user/running
 * ，输出：{"active":true,"address":"IP:11111","cid":2} cid address active
 * List<ZKDestinationClusterNode> zk命令 ：ls /otter/canal/destinations/ppdai_user/cluster
 * ，输出：[IP:11111, IP:11111] ZKDestinationClusterNode addressPort 对应 ： IP:11111 active
 * 程序计算，zk没有该状态，表示该canal server 是否是active ..... 第二个 canal server zKDestinationClientRunningNodePath
 * 对应zk路径 /otter/canal/destinations/ppdai_user/running ZKDestinationClientRunningNode
 * 消费该destination的client的信息，zk命令：get /otter/canal/destinations/ppdai_user/running
 * ，输出：{"active":true,"address":"IP:11111","cid":2} clientId address active
 ...
 * {"@type":"com.alibaba.otter.canal.protocol.position.LogPosition","identity":{"slaveId":-1,
 * "sourceAddress":{"address":"IP","port":3403}},"postion":{"included":false,"journalName":
 * "mysql-bin.000008","position":43508043,"serverId":123456,"timestamp":1528098351000}}
 * 
 */
public class ZKDestinationBean {
  ...
  private ZKDestinationRunningNode zKDestinationRunningNode;// 1：正在运行该destination的server节点 , get
                                                            // /otter/canal/destinations/ppdai_user/running
  private String zKDestinationClusterNodePath;

  private List<ZKDestinationClusterNode> zKDestinationClusterNodeList;// 2:
                                                                      // 部署该destination对应的cluster列表
                                                                      // , ls
                                                                      // /otter/canal/destinations/ppdai_user/cluster

  private ZKDestinationClientRunningNode zKDestinationClientRunningNode;// 3：消费该dest对应的canal
                                                                        // client运行在哪台节点上的信息， get
                                                                        // /otter/canal/destinations/ppdai_user/1001/running
  private String zKDestinationClientRunningNodePath;

  private LogPosition clientCursorLogPosition;// 4：该dest对应的client在zk的消费位点信息，get
                                              // /otter/canal/destinations/ppdai_user/1001/cursor
  ...
  public void setzKDestinationClusterNodeList(
      List<ZKDestinationClusterNode> zKDestinationClusterNodeList) {
  ...
  public void setzKDestinationClientRunningNode(
      ZKDestinationClientRunningNode zKDestinationClientRunningNode) {
