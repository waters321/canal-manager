...
public class SSHUtil {
  ...
  public static int executeCommand(HostInfo hostInfo, String command, boolean debugOutputEnabled)
      throws IOException {
    SSHClient ssh = getSshClient(hostInfo);
  ...
  private static SSHClient getSshClient(HostInfo hostInfo) throws IOException {
    ...
    for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        System.out.format("Failed to login to host %s as user %s. Exception: %s.%n", hostInfo.host,
            hostInfo.user, exception.getMessage());
        System.out.format("Attempt %d of %d. Sleeping for %d ms.%n", attempt, MAX_ATTEMPTS,
            sleepTimeMs);
    ...
    throw new RuntimeException(
        String.format("Unable to login to host %s as user %s after %d attempts.", hostInfo.host,
            hostInfo.user, MAX_ATTEMPTS));
  }
